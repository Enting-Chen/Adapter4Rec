...
def test(args, use_modal, local_rank):
    if use_modal:
        ...
        before_item_id_to_dic, before_item_name_to_id = read_news(
    ...
    if 'sasrec' in args.arch:
        ...
        model = ModelCPC(args, item_num, use_modal, bert_model).to(local_rank)
...
def setup_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
    torch.backends.cudnn.deterministic = True  # 加上这一行
    torch.backends.cudnn.benchmark = False  # 加上这一行
