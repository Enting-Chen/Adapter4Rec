...
def read_behaviors(behaviors_path, before_item_id_to_dic, before_item_name_to_id, max_seq_len, min_seq_len, Log_file):
    ...
    with open(behaviors_path, "r") as f:
        for line in f:
            ...
            # jump out the for loop to the next step
            if len(history_item_name) < min_seq_len:
                continue
            # get 23 items, 1-20 training to predict 2-21 to calculate the loss, 2-21 to validate 3-22, test: 3-22 to validate 4-23
    ...
    with open(news_path, "r") as f:
        for line in f:
            ...
            if 'abstract' in args.news_attributes:
                abstract = tokenizer(abstract.lower(), max_length=args.num_words_abstract, padding='max_length',
                                     truncation=True)
            else:
                abstract = []

            if 'body' in args.news_attributes:
                body = tokenizer(body.lower()[:2000], max_length=args.num_words_body, padding='max_length',
                                 truncation=True)
            else:
                body = []
            item_name_to_id[doc_name] = item_id
